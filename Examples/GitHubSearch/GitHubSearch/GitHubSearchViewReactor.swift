...
//                self.search(query: query, page: 1)
                self.searchWithMoya(query: query, page: 1)
    ...
    /// Moya를 이용한 API 호출
    private func searchWithMoya(query: String?, page: Int) -> Observable<(repos: [String], nextPage: Int?)> {
        let emptyResult: ([String], Int?) = ([], nil)
        guard let stringQuery = query else { return .just(emptyResult)}
        gitHubProvider.rx.request(.search(query: stringQuery, page: page))
//            .mapObject(ApiResponseModel<GitHub.search.Response>.self)
            .subscribe(onSuccess: { success in
//                print(success)
                let response = String(bytes: success.data, encoding: String.Encoding.utf8)
                print(response)
